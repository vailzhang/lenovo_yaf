$(document).ready(function(){
	...
	$("button#add_btn").bind("click",function(){
		...
		var subcate_id = $("#myModal").find("#parentsSelect").val();
		var a_color = $("#myModal").find("#a_colorInput").val();
		var b_color = $("#myModal").find("#b_colorInput").val();
		$(".span12").load("addNaviitem",{'title':title,'url':url,'subcate_id':subcate_id,'a_color':a_color,'b_color':b_color,'page':page},
				function(msg){
				});
	});
	/*点击修改后修改弹框内容变化为当前内容*/
	$("button#update").bind("click",function(){
		...
		var subcate_id= $.trim($(this).parents("tr").find("#subcate_id").val());
		...
		$("#myModal").find("#parentsSelect").find("option[value="+subcate_id+"]").attr('selected',true);		
	});
	/*修改弹框点击保存按钮*/
	$("button#update_btn").bind("click",function(){
		...
		var a_color = $("#myModal").find("#a_colorInput").val();
		var b_color = $("#myModal").find("#b_colorInput").val();
		$(".span12").load("updateNaviItem",{'id':id,'title':title,'url':url,'subcate_id':subcate_id,'a_color':a_color,'b_color':b_color,'page':page},
				function(msg){
				});
	});
	/*点击删除按钮*/
	$("button#del").bind("click",function(){
		...
		$.post("delNaviItem",
				...
				function(msg){
					if(msg == 'sucess'){
						window.location="naviitem?page="+page;
					}
				});
	});
	/*保存位置*/
			...
			setTimeout(function(){
				$("tbody.drag_tr.ui-sortable tr").each(function(){
					if($(this).find("td p").eq(0).text() == ThisId){
						...
						$.post("savePostionNaviItem",{'ThisId':ThisId,"PrevId":PrevId,"NextId":NextId},
								function(msg){
								});
					}
				});
